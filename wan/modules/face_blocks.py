...
import math
from .layer import MemoryEfficientAttention
...
class FlameAdapter(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.hidden_size = hidden_dim
        self.heads_num = heads_num or hidden_dim // head_dim
        
        ffn_dim = ffn_dim or hidden_dim * ffn_ratio
        self.proj_in = nn.Linear(flame_dim, hidden_dim)
        self.blocks = nn.ModuleList(
            [
                FlameBlock(self.hidden_size, self.heads_num, ffn_dim, head_dim)
                for _ in range(num_adapter_layers)
            ]
        )
...
class FlameBlock(nn.Module):
    def __init__(self, hidden_size: int, heads_num: int, ffn_dim: int, head_dim: int):
        super().__init__()
        self.attn = MemoryEfficientAttention(hidden_size, heads=heads_num, dim_head=head_dim)
