...
from ..util import checkpoint_wrapper
...
class ResidualBlock(nn.Module):
    ...
    @checkpoint_wrapper
    def forward(self, x, feat_cache=None, feat_idx=None):
        # Initialize feat_idx if None
        if feat_idx is None:
            feat_idx = [0]

        h = self.shortcut(x)
        for layer_idx, layer in enumerate(self.residual):
...
class Encoder3d(nn.Module):
    ...
    def forward(self, x, feat_cache=None, feat_idx=None):
        ...
        ## downsamples
        for layer_idx, layer in enumerate(self.downsamples):
            if feat_cache is not None:
...
class Decoder3d(nn.Module):
    ...
    def forward(self, x, feat_cache=None, feat_idx=[0]):
        ...
        ## upsamples
        for idx, layer in enumerate(self.upsamples):
            if feat_cache is not None:
                x = layer(x, feat_cache, feat_idx)
            else:
                x = layer(x)
...
class WanVAE_(nn.Module):
    ...
    def encode(self, x, scale):
        self.clear_cache()
        ## cache
        scale = [s.to(x.device).to(self.dtype) for s in scale]
        t = x.shape[2]
        iter_ = 1 + (t - 1) // 4
    ...
    def decode(self, z, scale):
        ...
        scale = [s.to(z.device).to(self.dtype) for s in scale]
        if isinstance(scale[0], torch.Tensor):
...
class WanVAE(nn.Module):
    ...
    def encode(self, videos):
        ...
        with amp.autocast("cuda", dtype=self.dtype):
            return [
                self.model.encode(u.unsqueeze(0), self.scale).squeeze(0).detach()
                for u in videos
            ]
