...
class FLAMEModel(nn.Module):
    ...
    def __init__(self, n_shape, n_exp, flame_version="2020", with_texture=False):
        ...
        self.flame_path = _abs_path
        self.flame_ckpt = torch.load(
            os.path.join(self.flame_path, f"flame_{flame_version}.pt"), weights_only=True
        )
        if with_texture:
            self.flame_texture_ckpt = torch.load(
                os.path.join(self.flame_path, f"flame_texture.pt"), weights_only=True
            )
        ...
        eye_vertices = (
            list(range(4477, 4594, 4)) + [4598, 4602, 4597] + list(range(3931, 4048, 4)) + [4052, 4056, 4051]
        )
    ...
    def get_flame_verts(self, motion_code, shape_code=None, with_headpose=True):
        ...
        assert motion_code.shape[-1] == 110
        batch_size, motion_length, _ = motion_code.shape
        exp_code, gpose_code, jaw_code, eyepose_code = motion_code.split([100, 3, 1, 6], dim=-1)
