...
class MicaArcfaceEncoder(nn.Module):
    def __init__(self, ):
        super().__init__()
    ...
    def encode(self, arcface_imgs):
        codedict = {}
        codedict['arcface'] = torch.nn.functional.normalize(self.arcface(arcface_imgs))
        return codedict

    def decode(self, codedict):
        identity_code = codedict['arcface']
        pred_shape_code = self.flameModel(identity_code)
        return pred_shape_code, identity_code
...
class MappingNetwork(nn.Module):
    ...
    def forward(self, z):
        ...
        for i, l in enumerate(self.network):
            h = self.network[i](h)
            h = torch.nn.functional.leaky_relu(h, negative_slope=0.2)
            if i in self.skips:
                h = torch.cat([z, h], 1)
...
class Generator(nn.Module):
    def __init__(self, z_dim, map_hidden_dim, map_output_dim, hidden):
        super().__init__()
