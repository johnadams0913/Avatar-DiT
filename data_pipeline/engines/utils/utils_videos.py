...
def read_all_video_frames(video_path, device='cpu', target_shape=None):
    container = av.open(video_path)
    video_stream = next((s for s in container.streams if s.type == 'video'), None)
    if video_stream is None:
    ...
    for frame in tqdm(container.decode(video=0), total=total_frames, desc="Reading video frames"):
        if frame.pts is None:  # Ignore invalid frames
            continue
        frame = torch.from_numpy(frame.to_ndarray(format="rgb24")).permute(2, 0, 1).to(device).float()
        if target_shape is not None:
            frame = torchvision.transforms.functional.resize(frame, target_shape, antialias=True)
        frames.append(frame)
...
def read_audio_samples(video_path, stero=False):
    ...
    if audio_data.max() > 1.05 or audio_data.min() < -1.05:
...
def read_video_frames_async(video_path, buffer_size=10, device=None):
    def frame_loader(frame_queue, container):
        try:
            for frame in container.decode(video=0):
    ...
    loader_thread = threading.Thread(target=frame_loader, args=(frame_queue, container))
