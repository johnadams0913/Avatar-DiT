...
class TrackCropEngine:
    ...
    def run_video_pipeline(self, video_path):
        ...
        for track in meta_data['all_face_data']:
            # start_time = track['start_frame'] / video_meta_data['video']['frame_rate']
            # end_time = track['end_frame'] / video_meta_data['video']['frame_rate']
            left, top, width, height, _ = track['bboxes'][0]
            left, top, width, height = int(left), int(top), int(width), int(height)
            left, top, width, height = expand_bbox((left, top, width, height), video_meta_data['video']['height'], video_meta_data['video']['width'], 1.8)
            output_path = f'{self.output_dir}/videos/{video_name}.mp4'
            ...
            if os.path.exists(output_path):
                continue
            video_stream = ffmpeg.input(video_path).video.crop(left, top, width, height)
            audio_stream = ffmpeg.input(video_path).audio
            ffmpeg.output(
                video_stream, audio_stream, output_path,# overwrite_output=True,
                r=video_meta_data['video']['frame_rate'], vcodec='libx264', acodec='aac', ar=16000, strict='experimental' # copy or h264_nvenc or libx264
...
def expand_bbox(bbox, image_height, image_width, bbox_scale=1.42):
    ...
    if bbox_scale < 0:
        # 当bbox_scale < 0时，将bbox扩展到图像短边的尺寸
